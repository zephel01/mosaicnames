...
低解像度(非 Retina など)のスクリーンショットは文字が潰れて OCR が
取りこぼしやすいため、長辺が 1200px 未満の画像は OCR にかける直前だけ
拡大しています。同じ画像内でも倍率によって読み取れる箇所にばらつきが
出ることがあるため、既定では 2倍・3倍の両方で OCR して検出結果を合算
します(`--upscale-threshold` / `--upscale-factor` で調整)。実際に
モザイクをかける画像は常に元の解像度のままです。

## ⚠️ 注意

- OCR ベースのため、極端に小さい文字・低コントラスト・装飾フォントは
  取りこぼすことがあります。自動拡大はこれを軽減しますが完全ではないため、
  **公開前に `--list` と目視での最終確認を推奨**
